...
void loop(){ 
  ...
  /*
//Need this function for calibrate
   for (i = MinSensor ; i <= MaxSensor ; i = i + 1) {
   Serial.println(flexSensorReading[2]);                                     // Raw data before mapping
   } 
   */

  //Calibration
  flex0to100_sensor[1] = map(flexSensorReading[1], 950, 1000, 0, 100);        // Mapping Raw data from 0 to 100 for A0
