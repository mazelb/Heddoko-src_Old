...
boolean[] first = { true, true, true, 
                    true, true, true, 
                    true, true, true }; //9 Sensor points (9 Lilypads)

// One Rotation object per sensor to keep track of each orientation (default: no rotation)
Rot[] vOrientations = { new Rot(new PVector(1,0,0), 0), 
                        new Rot(new PVector(1,0,0), 0), 
                        new Rot(new PVector(1,0,0), 0),
...
boolean equalsPort(Serial first, Serial second) {
  return true;//first.port.getName().equals(second.port.getName());
}
